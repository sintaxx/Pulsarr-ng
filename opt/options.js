...
class Server {
	...
	get(endpoint, params = "") {
		...
		return new Promise(function(resolve, reject) {
			...
			// Radarr v6 / Sonarr v4 use /api/v3
			if (endpoint && endpoint.startsWith("/api/") && !endpoint.startsWith("/api/v")) {
				endpoint = endpoint.replace("/api/", "/api/v3/");
			}
			...
			// Basic auth can be boolean true/false or string "true"
			if (self.auth === true || self.auth === "true") {
				http.setRequestHeader("Authorization", "Basic " + btoa(self.user + ":" + self.password));
			}
